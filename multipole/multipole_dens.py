...
class Multipoles(AnalysisBase):
    ...
    def __init__(self, select, centre='M', grouping='water', axis='z', ll=0, ul=None, binsize=0.25, H_types=['H'], type_or_name=None, **kwargs):
        ...
        if self.ll > self.ul:
            raise ValueError(
                f'Lower limit of axis {self._axind} must be smaller than the upper limit!')
    ...
    def _prepare(self):
        ...
        self.Matoms = self._universe.select_atoms(
            f'{self.type_or_name} {self.centretype}')
        self.Hatoms = self._universe.select_atoms(
            f"{self.type_or_name} {' '.join(self.H_types)}")  # joining all the H types with a string
    ...
    def _single_frame(self):
        ...
        # TODO generalise by making this charge a vector, can use with reaxff then.
        qH = self.Hatoms.charges[0]
        ...
        # first axis is atoms, second axis are the cartesian compnents
        ...
        # print(shape(zposes))
        mu_hist = np.vstack([np.histogram(
            zMw, bins=self.nbins, weights=mus[:, i], range=self.range)[0] for i in range(3)])
        Q_hist = np.stack([[np.histogram(zMw, bins=self.nbins, weights=Q[:, i, j], range=self.range)[
                          0] for i in range(3)] for j in range(3)])

        mol_hist = np.histogram(zMw, bins=self.nbins, range=self.range)[
            0]  # un weighted!

        # NB These are divided by the number of molecules in each bin; i.e., average of cos(theta) per bin
        angle_hist = np.nan_to_num(np.histogram(
            zMw, bins=self.nbins, weights=cos_theta, range=self.range)[0]/mol_hist)
        ...
        # convert nans in profiles to zeros -> assumes that if density is zero then can't have any angles!!!!
        angle_sq_hist = np.nan_to_num(np.histogram(
            zMw, bins=self.nbins, weights=cos_moment_2, range=self.range)[0]/mol_hist)
        # print(Q_hist.shape)
    ...
    def _conclude(self):
        ...
        for prop in ['charge_dens', 'dipole', 'quadrapole']:
            # divide by the volume and number of frames used and convert to Coloumbs per unit
            prop_val = getattr(self, prop)/self.n_frames / \
                self.slice_vol * elementary_charge
        ...
        for prop in ['cos_theta', 'angular_moment_2']:
            # divide by the number of frames used. NB. Already divided by the number of molecules per chunk.
            prop_val = getattr(self, prop)/self.n_frames
            setattr(self, prop, prop_val)
    ...
    def check_types_or_names(universe):
        try:
            ...
            try:
                ...
                raise mda.NoDataError(
                    'Universe has neither atom type or name information.')
        return type_or_name
